  ...
  return (
    <div className="max-w-lg mx-auto px-4 pt-6 pb-4">
      ...
      <div className="flex items-center justify-between mb-4">
        <h1 className="font-heading text-2xl font-bold text-foreground">Meal Planner</h1>
        <Link
      ...
      {/* Week navigation */}
      <div className="flex items-center justify-between bg-card rounded-2xl border border-border shadow-sm px-4 py-3 mb-5">
        <button
        ...
        <div className="space-y-2 mb-6">
          {DAYS.map((day, i) => {
            ...
            return (
              <div
                key={day}
                className={`bg-card rounded-2xl border shadow-sm overflow-hidden active:scale-[0.99] transition-all ${
                  isToday ? 'border-brand/40' : 'border-border'
                }`}
              >
                <div className={`flex items-center ${slot ? 'justify-between' : ''}`}>
                  <div className={`px-4 py-3 min-w-[72px] ${isToday ? 'text-brand' : 'text-muted-foreground'}`}>
                  ...
                  {slot ? (
                    <>
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <p className="text-sm font-semibold text-foreground line-clamp-1">{(slot.recipe as any)?.name}</p>
                    ...
                    <button
                      onClick={() => setPickingDay(i)}
                      className="flex-1 px-4 py-3 text-left text-sm text-muted-foreground hover:text-brand hover:bg-brand-subtle/50 transition-colors"
                    >
                      + Add a meal
                    </button>
                  )}
                ...
                <li key={recipe.id}>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-foreground text-sm truncate">{recipe.name}</p>
